...
DeWatermark removes watermarks, logos, and unwanted objects from videos using the state-of-the-art [ProPainter](https://github.com/sczhou/ProPainter) deep learning model — specifically engineered to run on **low-VRAM GPUs (4–6 GB)** without crashing.
...
Most video inpainting tools load an entire video into GPU memory — impossible on consumer GPUs. DeWatermark was built from the ground up with a **disk-first, chunked-inference** architecture:

| Challenge | Standard Approach | DeWatermark |
|-----------|------------------|------------|
...
When `--resize auto` (default), DeWatermark picks a safe inference resolution based on your VRAM budget:
...
Yes. DeWatermark never loads the full video into memory. Processing time scales linearly with frame count. For a 1080p video downscaled to 480p on a 6 GB GPU, expect roughly 2-5 seconds per frame.
</details>
...
DeWatermark is built on top of the excellent [ProPainter](https://github.com/sczhou/ProPainter) model by Zhou et al. All inpainting credit belongs to the original authors.
